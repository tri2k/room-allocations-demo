...
**Phase 2 (next):** Google OAuth, public HTTPS, orgs. Org owns the catalog. Event is a label. Each user owns private sheets. No Share button and no live sync yet. Rooms are still not owned by Events — sheets book rooms from the org catalog, with an optional building filter on the sheet.
