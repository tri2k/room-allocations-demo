...
- **Swire is external:** proctor suite (timer, clarifications, projector, room password) is not a folder of this API. Swire’s exposed API is **read-only**. HQ may display timer state and must not start, pause, add time, or send clarifications. **Live stays** on this platform. No `/api/v1/swire/`. No room cookie on this API.
