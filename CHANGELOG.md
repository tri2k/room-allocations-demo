...
- **One event id** inside this platform (planner, HQ, volunteers, live). Each site keeps its own past data under that id. There is no cross-subdomain event toggle. Roomsdb is not per event. Swire may store the id as a label; it does not share the row. Which admin edits the shared facts is still open.
