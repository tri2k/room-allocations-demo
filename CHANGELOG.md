...
- **Auth:** volunteers OAuth is decided. **Live `/admin` is one shared account.** HQ, the planner, and roomsdb lean toward **one staff identity** (same person as their volunteer account), not three logins. A cookie still does not cross hosts. Whether one prompt covers those three hosts is open. Do not set the cookie on `.berkeley.mt`.
