...
- **One event id** inside this platform. Staff admins on volunteers, live, the planner, and HQ each have a selector and a create button on the same table. Creating on one host shows up on the others. There is no cross-site toggle. Roomsdb and Swire do not insert rows. Duplicate names, live’s weak password, and edit-after-create are the pitfalls.
