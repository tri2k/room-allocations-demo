...
- Account structure **open** with recommendation **A:** named identity is `Person` (officers fill the volunteer form). Ops is a **staff password** door, not a second biography. One-login-with-flag (B) if officers refuse two secrets. Room login stays the laptop.
