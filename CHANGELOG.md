...
- **One person, one permissions page.** A superuser sets per-platform access (volunteer admin versus ordinary volunteer, and access to roomsdb, the planner, and HQ). Platform admins edit that platform. They do not get a second user-management screen inside it. The first superuser is an allowlisted email. View versus edit on each internal tool is still open.
