...
- Persistence: **one** Postgres 16, **one** database (`roomalloc`). Six modules are table groups + UIs that join `rooms.id`, not extra databases. Student registration stays an external product (CSV in). Figma `.fig` is import source, not a runtime DB. Alternatives (per-module DBs, extra instances, schemas, replicas) recorded; per-module DBs are last semester’s failure mode.
