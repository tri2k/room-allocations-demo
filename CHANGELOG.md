...
- **Hosts:** keep **`swire.berkeley.mt`** (room clocks only; historical `/admin` moves to ops) and **`ops.berkeley.mt`** (volunteers + HQ dashboard). **`live.berkeley.mt`** for guests. Do not invent `hq.berkeley.mt`.
