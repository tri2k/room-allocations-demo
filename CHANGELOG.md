...
- **HQ reads other systems.** The Saturday page writes the day plan and the roster only. Roomsdb, maps, volunteers (`volunteers.berkeley.mt/admin`), live (`live.berkeley.mt/admin`), and Swire (Swire admins) are each edited on that system’s own admin.
