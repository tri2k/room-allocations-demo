...
- **Swire is external:** proctor suite (timer, clarifications, projector, room password) is not a folder of this API. Swire exposes an API. HQ calls it and degrades if it is down. **Live stays** on this platform. No `/api/v1/swire/`. No room cookie on this API.
