...
- **API paths:** `/api/v1`. Guest live is namespaced (`GET /api/v1/live`, `POST /api/v1/live/email/subscribe`). Roomsdb is resource paths (`/buildings`, `/rooms`), not one `/api/roomsdb` blob. Auth is cookie + route, not the prefix.
