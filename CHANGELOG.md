...
- **Staff is a flag on the person**, not an organization product. Signup does not grant it. The first person is one allowlisted email and can turn the flag on for others in the app, instead of editing the database. Whether only that person can grant staff afterward is open. Re-login from the planner to roomsdb is acceptable. Whether the person who may plan may also edit rooms is open.
- **Maps lean:** the floor plan lives with the rooms, year-round. Roomsdb is where you upload a replacement. HQ and live only view it. Upload permission is the same open question as editing rooms. How a Figma file becomes that drawing is still open.
