...
- **API paths:** `/api/v1`. Guest live is namespaced (`GET /api/v1/live`, `POST /api/v1/live/email/subscribe`). Roomsdb resources are `/api/v1/roomsdb/buildings` (etc.), not a single dump and not a second copy under ops. Auth is cookie + route, not the prefix.
