...
- **API paths:** `/api/v1/{module}/...` (roomsdb, ops, volunteers, swire, live). One owner per resource — HQ reads `/roomsdb/rooms`, no `/ops/rooms`. Prefixes are folders in one process, not six APIs. Flat REST and per-host unprefixed `/api` are the rejected alternatives.
