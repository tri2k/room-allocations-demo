...
- **One event id** inside this platform (planner, HQ, volunteers, live). Each site keeps its own past data under that id. There is no cross-subdomain event toggle. Roomsdb is not per event. Swire may store the id as a label; it does not share the row. Which admin creates the event is still open: volunteers, the planner, HQ, live, and a new setup page are each a bad fit for a different reason.
- **Auth:** volunteers OAuth is decided (signup, profile, and that host’s admin). **Live `/admin` is one shared account**, not OAuth. HQ, planner, and roomsdb are not decided. A volunteer cookie does not open those hosts.
