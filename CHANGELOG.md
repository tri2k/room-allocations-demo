...
- Rooms catalog is the kernel (**roomsdb**); allocator, day-of ops, proctors, public maps, and volunteers are modules that join `rooms.id`
...
- **Staff cadence:** roomsdb is rare (`roomsdb.berkeley.mt`); planner is regular (`ops.berkeley.mt/planner`); HQ is Saturday (`ops.berkeley.mt` root). They do not share primary chrome. Same Google, same API. **roomsdb** is the module name and the host; “catalog” in older sentences is the same thing.
