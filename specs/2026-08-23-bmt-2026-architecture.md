...
| 5 | **Public** | **`live.berkeley.mt`** | None on `/`. **`/admin`**: one shared admin account, cookie `Path=/admin` | Students, parents, coaches on `/`. Whoever has the admin account | Guests: read announcements and **maps**. Admin: edit public content (announcements) | Guest login button. Public countdown. Volunteer apply. Officer menus on `/`. Per-person OAuth on `/admin`. HQ, roomsdb, or volunteer admin inside `/admin`. Calling Swire |
